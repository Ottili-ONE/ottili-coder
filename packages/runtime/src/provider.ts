...
/**
 * A scripted tool call may be built from the request it answers. Durable ids
 * are generated by the control plane, so a fixed script cannot name a task or
 * agent that did not exist when the script was written — which is exactly the
 * situation a real model is in before it reads the graph.
 */
export type ScriptedToolCalls =
  readonly ToolCall[] | ((request: ProviderTurnRequest) => readonly ToolCall[]);

export type ScriptedProviderStep =
  | { readonly type: "text"; readonly text: string; readonly usage?: TurnUsage }
  | {
      readonly type: "tool_calls";
      readonly toolCalls: ScriptedToolCalls;
      readonly usage?: TurnUsage;
    }
...
export class ScriptedProvider implements TurnProvider {
  ...
  public async complete(
    ...
    while (true) {
      ...
      const toolCalls =
        typeof step.toolCalls === "function"
          ? step.toolCalls(request)
          : step.toolCalls;
      return step.usage === undefined
        ? { toolCalls }
        : { toolCalls, usage: step.usage };
    }
  }
}
